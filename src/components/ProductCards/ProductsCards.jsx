...
const ProductsCards = () => {
    const {cartIds, setCartIds, foodsItems} = useContext(AuthContext);
...

                {
                    foodsItems?.map(item => <Card sx={{ background: "#ebedef" }} key={item?.id}>
                        ...
                        <div className="p-4 space-y-4">
                            {
                                cartIds?.includes(item?.id) ? <Button disabled variant="contained" sx={{width: "100%", background: "#fd5442"}}>Added To Cart</Button> : <Button onClick={() => handleAddToCart(item?.id)} variant="contained" sx={{width: "100%", background: "#fd5442"}}>Add To Order</Button>
                            }
